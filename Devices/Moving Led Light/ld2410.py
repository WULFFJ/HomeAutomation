...
ser = UART(1, baudrate = 256000, tx=Pin(21), rx=Pin(20), timeout = 1)
serial_status = False
...
def run_forever():
    ...
    while True:
        ...
        if meas['state'] == STATE_MOVING_TARGET or meas['state'] == STATE_COMBINED_TARGET:
            if meas['moving_distance'] <= 300:
                led1.value(1)
                print(meas)
                if 2 not in led_order:
                    led_order.append(2)
            if meas['moving_distance'] <= 200:
                led2.value(1)
                if 3 not in led_order:
                    led_order.append(3)
            if meas['moving_distance'] <= 125:
                led3.value(1)
                if 4 not in led_order:
                    led_order.append(4)
